...
const Index = () => {
  ...
  return (
    <div className={styles.container}>
      <h1 className={styles.title}>Pay back the loan</h1>
      {networkId === 1 && (
        <>
          <br />
          <div>
    ...
    <div>
        <label>
            <span className={styles.label}>Borrower token name</span>
    ...
    <div>
      <label>
        <span className={styles.label}>Loan TX hash (not request hash)</span>
        <input
          className={styles.input}
          type="text"
          value={tx.loanTx}
          onChange={(e) => tx.setLoanTx(e.target.value?.toString())}
        />
      </label>
    </div>

      <div>
        <button disabled={!canTransact} className={styles.button} onClick={tx.paybackLoan}>
          Send transaction
        </button>
        <label>
          <br></br>
